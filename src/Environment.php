 ...
 * be common across multiple lee applications.
 *
...
class Environment implements \ArrayAccess, \IteratorAggregate {
     ...
     * variables by using `\Lee\Environment::mock()` instead.
     *
    ...
    public static function mock($userSettings = []) {
        self::$environment = new self(array_merge(self::getDefaultProperties(), $userSettings));
        return self::$environment;
    }
    ...
    protected static function getDefaultProperties() {
        return [
            ...
            'USER_AGENT'      => 'Lee Framework',
            'REMOTE_ADDR'     => '127.0.0.1',
            'lee.url_scheme' => 'http',
            'lee.input'      => '',
            'lee.errors'     => @fopen('php://stderr', 'w'),
        ];
    }
    ...
    private function __construct($settings = null) {
        if ($settings) {
            ...
            $env['lee.url_scheme'] = empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] === 'off' ? 'http' : 'https';
            ...
            $env['lee.input'] = $rawInput;

            //Error stream
            $env['lee.errors'] = @fopen('php://stderr', 'w');

            $this->properties = array_merge(self::getDefaultProperties(), $env);
        }
    }
