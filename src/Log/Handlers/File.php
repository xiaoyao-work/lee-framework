...
class File {
    ...
    public function write($log, $level, $destination = '') {
        ...
        $filename    = empty($destination) ? 'common' : $destination;
        if (strpos($destination, '/') !== false) {
            $destination = $this->config['log_path'] . '/' . trim($filename, '/') . '.log';
        } else {
            $now         = date($this->config['log_time_format']);
            $destination = $this->config['log_path'] . '/' . $now . '/' . $level . '/' . $filename . '.log';
        }
